...
- "<lora:loraName:0.8>" notation for LoRA models and weights (<lora:loraName:loraWaight>)
- JSON blocks that might already exist in the AI response

If the prompt itself mention the lora with such notation : <lora:loraName:0.8>, remove it fromt the prompt
Example output format:
{
  ...
  "data": {
    ...
    "height": number | 512;
    "tags": ["relevant", "tags", "here"]
    "loras": [{name:(Must be a name from the available loras, not label),weight: number}], make sure to put here only the models tagged as loras;
  }
}
